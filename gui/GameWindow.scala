...
class GameWindow {
    ...
    private val tris = GameWindow.createTetrahedron()
    private val pitchAngle = Array(0.0)
    private val headingAngle = Array(0.0)
    ...
    def launch(): Unit = {
        SwingUtilities.invokeLater(() => {
            ...
            frame.add(createRenderPanel(), BorderLayout.CENTER)
            frame.setVisible(true)
        })
    }

    private def createRenderPanel(): JPanel = {
    val panel = new JPanel() {
        ...
        val dx = Math.toRadians(headingAngle(0))
        val dy = Math.toRadians(pitchAngle(0))
    ...
    panel.addMouseMotionListener(new MouseMotionListener {
        ...
        val xi = 180.0 / (panel.getWidth /100)
        val yi = 180.0 / (panel.getHeight /100)
